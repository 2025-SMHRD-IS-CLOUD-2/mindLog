          ...
          data.forEach((element) => {
            ...
            scheduleChange.addEventListener("click", function (e) {
              ...
              const appointmentInfo =
                e.currentTarget.closest(".appointmentInfo");
              const changeDiv =
                appointmentInfo.querySelector(".change-date-div");
            ...
            appoint.addEventListener("click", function (e) {
              ...
              if (currentlySelected === clicked) {
                ...
                // 이외의 것을 선택하면 기존 선택된 것의 classList에서 selected를 제거
...
function changeDate(appoint, appointmentSeq, centerSeq) {
  ...
  timeSelect.innerHTML =
    "<option disabled selected>변경일을 먼저 선택하세요</option>";
...
function calendar(year, currMonth, today) {
  ...
  for (let i = 1; i <= tMLD.getDate() + tMFD.getDay(); i++) {
    if (i <= tMFD.getDay()) {
      // 저번달 날짜들 이번달 첫날 앞에 출력
      let days = document.createElement("div");
      days.setAttribute("class", "days");
      days.innerHTML = `<span class = 'LMD'>${
        lMLD.getDate() - tMFD.getDay() + i
      }</span>`;
...
for (let i = 0; i < changeMonth.length; i++) {
  // 월을 바꾸면 그에 맞는 달력으로 변환
